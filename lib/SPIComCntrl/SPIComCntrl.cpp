...
static float filtered_setpoint = 0.0f;
...
void SPIComCntrl::executeTask()
{
    ...
    //temporary code for setpoint sequence
        ...
        current_setpoint = setpoint_sequence[step_index];
    ...
    if (m_Imu.isCalibrated()) {
        ...
        if ((missing_data_counter * m_Ts) <= VISION_TIMEOUT) {
            ...
            //sollwert tiefpass
            static constexpr float SETPOINT_TAU = 0.1f; // Tf aus MATLAB (Ca. 1 / w_d)

            // Weichzeichnen des Sollwerts
            float alpha_sp = m_Ts / (SETPOINT_TAU + m_Ts);
            filtered_setpoint = filtered_setpoint + alpha_sp * (current_setpoint - filtered_setpoint);
            
            // Calculate error between desired postion and current ball position
            float error_x = filtered_setpoint - m_spiData.data[0]; //input in mm
            float error_y = filtered_setpoint - m_spiData.data[1]; //input in mm
            ...
            ikInput.h = 98.1f; // original height for middle pos
            ...
            // ikInput.h = 98.1f - compensation * 1.0f;
    ...
    if (m_SerialStream.startByteReceived()) {
        ...
        m_SerialStream.write(m_spiData.data[0]); //  1 Echo servo D0 command
        m_SerialStream.write(m_spiData.data[1]); //  2 Echo servo D1 command
        m_SerialStream.write(filtered_setpoint); //  3 Echo servo D2 command
