                           ...
                           int baudrate) : _bufferSize(sizeof(float) * S_STREAM_CLAMP(num_of_floats))
{
    ...
    // Configure DMA stream for USART1_TX (F446RE: DMA2 Stream7, Channel4)
    _txDma.Instance = DMA2_Stream7;
    ...
    // Configure DMA stream for USART1_RX (F446RE: DMA2 Stream2, Channel4) in circular mode
    _rxDma.Instance = DMA2_Stream2;
    ...
    // NVIC priorities
    NVIC_SetVector(DMA2_Stream7_IRQn, reinterpret_cast<uint32_t>(serialStreamDmaTxIrq));
    NVIC_SetPriority(DMA2_Stream7_IRQn, DMA_IRQ_PRIORITY);
    NVIC_EnableIRQ(DMA2_Stream7_IRQn);

    NVIC_SetVector(DMA2_Stream2_IRQn, reinterpret_cast<uint32_t>(serialStreamDmaRxIrq));
    NVIC_SetPriority(DMA2_Stream2_IRQn, DMA_IRQ_PRIORITY);
