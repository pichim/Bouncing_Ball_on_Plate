...
class CameraProcessor:
    def __init__(self):
        ...
        # =======================================================
        self.RECORDING_DURATION = 1.3  # Angepasst an die 3.0s des i-Loops
        self.STAMP_INTERVAL_MS = 60.0  # Alle wie viel Millisekunden wird gestempelt? (z.B. 60ms)
        
        # =======================================================
        # --- EINSTELLUNGEN FÜR BLINDZONE (i-Pünktchen Hack) ---
        # =======================================================
        self.ENABLE_BLIND_ZONE = True   # True = Blendet einen Y-Bereich aus
        self.BLIND_Y_MIN = 300          # Pixel-Grenze OBEN (kleinerer Wert = weiter oben)
        self.BLIND_Y_MAX = 500          # Pixel-Grenze UNTEN (größerer Wert = weiter unten)
        ...
        if contours:
            ...
            if radius > 5:
                ...
                if time_since_last_stamp_ms >= self.STAMP_INTERVAL_MS:
                    ...
                    if self.ENABLE_BLIND_ZONE:
                        # center[1] ist die aktuelle Y-Pixel-Koordinate des Balls
                        if self.BLIND_Y_MIN <= center[1] <= self.BLIND_Y_MAX:
                            is_in_blind_zone = True

                    # Nur stempeln, wenn der Ball NICHT in der Blindzone ist
                    if not is_in_blind_zone:
                        self.last_stamp_time = current_time
